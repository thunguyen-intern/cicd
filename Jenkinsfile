...
void deployToHost(host, version, oppositeVersion) {
    withEnv(["DOCKER_HOST=${host.host}"]) {
        ...
        if (http_code == "200"){
            ...
            println("---------------------------------")
            sh """docker run --privileged=true ${host.container}_${oppositeVersion} sh -c \"sudo apt-get install -y nfs-common nfs-utils && showmount -e odoo_data && mount -t nfs -o nolock odoo_data:/opt/odoo/.local/share/Odoo/ /home/odoo/.local/share/Odoo/\""""
