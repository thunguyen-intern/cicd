...
void deployToHost(host, version, oppositeVersion, firstServerContainer) {
    withEnv(["DOCKER_HOST=${host.host}"]) {
        sh """docker run -ti --name=${host.container}_${version} -v Filestore:/home/odoo/.local/share/Odoo -h ${host.container}_${version} -d --volumes-from ${firstServerContainer} --network odoo ${DOCKERHUB_CREDENTIALS_USR}/${IMAGE}:${ID}"""
